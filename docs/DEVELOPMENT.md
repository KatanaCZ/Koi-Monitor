...
- **Dev** (console) : `koiSimulateLoad()` · `koiSimulateGaming()` (30 s) · `koiSimulateLatency200()` · `koiSimulateZen()` · `koiSimulateFlow()` · `koiSimulateBoost()` · `koiSimulateFocus(lostFocus?: boolean)` · `koiPlayEasterMusic()` · `koiStopEasterMusic()` — voir `alertDevSimulation.ts` / `easterEggMusic.ts`.
